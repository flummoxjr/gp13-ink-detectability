...
**One measurement caveat before the counts are used.** The angle compares a whole-mesh average normal against the sheet normal in one central cube, and a large, strongly curved mesh can score high on curvature alone — the human-traced w032 control reads 59.2° under the identical method, and no one thinks w032 fails to follow its papyrus. Our own PHerc0813 result is immune to this (recomputed on only the vertices inside the sampled cube it gets *worse*, 72.9°), but the corpus-wide "19 of 56 at ≥ 45°" should be read as an upper bound on the defect, not a census of it; the locally-restricted recompute over the already-cached cubes is the cheap upgrade and is queued.
